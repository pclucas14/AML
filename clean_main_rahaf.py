        ...
        for i, (data, target, idx) in enumerate(tr_loader):
            ...
            for it in range(args.disc_iters):
                ...
                if args.method == 'mask':
                    ...
                    for t in target:
                        buffer.class_freq[t]+=1

                elif args.method == 'triplet':
                    # fetch the constrasting points
                    pos_x, neg_x_same_t, neg_x_diff_t, invalid_idx, y_pos, y_neg_same, y_neg_diff = \
                    ...
                    #    print(neg_hid_same_t.shape)
                     #   print(normalize(model.linear.L.weight[target[~invalid_idx]]).shape)
                        ...
                        loss += 2.0 * F.triplet_margin_loss(hidden_norm,
                                                            pos_hid,
                                                            normalize(model.linear.L.weight[y_neg_diff[~invalid_idx]]),0.2)
                ...
                if rehearse:
                    mem_x, mem_y, bt, inds = buffer.sample(args.buffer_batch_size, aug=args.augment, ret_ind=True)#, exclude_task=task)
                  #  pos_hid, neg_hid_same_t,neg_hid_diff_t = all_hid[:, :].chunk(3)
                    for t in mem_y:
                        buffer.class_freq[t]+=1
                   # mem_x = torch.cat((neg_hid_same_t.squeeze(),neg_hid_diff_t.squeeze()))
                    #mem_y = torch.cat((y_neg_same, y_neg_diff))
