...
for run in range(args.n_runs):
    ...
    buffer = Buffer(args.input_size, args.n_classes, args.mem_size).to(args.device)
    if run == 0:
    ...
    for task, tr_loader in enumerate(train_loader):
        ...
        for i, (data, target, idx) in enumerate(tr_loader):
            ...
            buffer.add(data, target, task, idx)
            #------ Train Classifier-------#
            if i==0:
                print('\n--------------------------------------')
            ...
            for it in range(args.disc_iters):
                ...
                if rehearse:
                    mem_x, mem_y, bt, inds = buffer.sample(args.buffer_batch_size, aug=False)
